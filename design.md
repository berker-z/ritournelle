...
- **Project wiring**: `project.godot` autoloads `SaveSystem`, `GameState`, `EncounterSystem`, `CraftingSystem`; main scene `scenes/Main.tscn` is a text harness with log/status and a Map button. Navigation uses full-screen overlays: `MapPanel` (choose town/lake/forest/mountain), `TownPanel`, `ZonePanel` (list nodes, move to node, rest/craft), and `NodePanel` (harvest/combat/return/rest/craft). A reusable `ActionBar` (Inventory/Skills/Save & Exit) sits on Main and all overlays. Inventory/Skills remain full-screen overlays; logic is being pulled into small controllers.
...
- **Maps/Encounters** (`data/maps.gd`, `map_node.gd`, `encounter_system.gd`): Submaps `town`, `lake`, `forest`, `mountain`; town has no nodes. Travel costs: `SUBMAP_TRAVEL_COSTS` (lake/forest/mountain 10, town 10). Node movement costs: `abs(distance diff) * SUBMAP_NODE_COSTS[submap]` (lake/forest 2, mountain 3). Node actions (harvest/combat) consume node energy; travel cost is separate. XP routing: harvest XP by submap (harvest._), combat XP by equipped weapon skill (combat._) or unarmed if no weapon.
...
- Account/character selection moved out of Main into `AccountPanel` (full-screen, opaque) plus `AccountController` that mediates create/select/delete and visibility. Inventory/skills flows mediated by `InventorySkillsController`; TownPanel intents mediated by `TownController`.
...
- Controllers: `AccountController` + `AccountPanel` handle account/character create/select/delete; `InventorySkillsController` mediates inventory/skills open/refresh/equip/unequip/save; `TownController` mediates town intents. Main now delegates these flows instead of wiring every button directly.
