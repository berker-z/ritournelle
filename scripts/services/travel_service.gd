...
func list_submaps() -> Array[String]:
	...
	subs.append_array(submap_nodes.keys())
	subs.sort()
	return subs
...
func travel_to_submap(player, submap: String) -> Array[String]:
	...
	# If in sam submap but at node, return to entrance logic
...
func move_to_node(player, target_submap: String, node_id: String) -> Array[String]:
	...
	if get_submap_from_location(player.location) != target_submap:
		return ["Travel to %s first." % target_submap]
	...
	if target_distance <= 0:
		return ["Node %s is not in %s." % [node_id, target_submap]]
	...
	return ["Moved to %s (-%.1f energy)." % [node_id, total_cost]]
